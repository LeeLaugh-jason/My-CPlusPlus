...
* 叔叔节点是黑色（或NIL），且z是“内侧”节点（LR或RL）：LR型指z节点是p节点右子节点，p是g的左子节点。PL同理。

将父节点进行一次旋转（LR先左旋p，RL先右旋p），再转化为上一种情况。再对祖父节点做一次旋转，最后将插入节点染黑，祖父节点染红：
...
总结来说，红黑树规定了一系列旋转和染色规则，使红黑树趋于平衡的状态。

而STL中的std::set就是基于这一种树的关联式容器。为何选择一种呢？因为这一种树旋转次数较少，颜色也比较好储存。

在初始化set时，会自动进行数据的去重，支持插入、查找、删除（时间复杂度O(logn)）和遍历（时间复杂度O(n)）元素，。这里插入查找删除遍历等都无法通过序列号进行，而是直接通过数据的内容插入和查找的，因为在树中没有了序列号等概念了。

由于set是有序的容器，我们可以直接从begin和rbegin获取二叉树中的最大值和最小值，时间复杂度是O(1)。

但是由于set在堆中的分布是不连续的，在CPU访问内存的时候进行**缓存命中**的概率反而比vector和array更小，在小量的数据中，反而vector+sort+unique会比set访问更快。

我们在写程序的时候，总是希望程序可以运行的更快。但数据毕竟保存在内存区中，离CPU有一段距离，每次都去访问肯定会很慢。于是CPU有了缓存（Cache）这个东西。在现在的CPU中有三级缓存L1、L2、L3，当程序需要读取数据的时候，会将它与它之后的数据一起读取到缓存行（Cache Line通常是64字节）中。这样，CPU在需要数据的时候，首先从三级缓存中依次读取，如果都没有才会区读取主存里面的数据，并且一次读取一整块，将这一块数据放入缓存中。

set中每一个节点在堆中随机分配，在遍历的时候每次跳转都可能跳转到不同的**内存页**中，**缓存未命中率**高。这里的内存页就是我们刚刚说到的一块一块固定大小的内存，通常是4KB。操作系统将进程的虚拟地址空间划分为一个一个页，物理内存也被划分为同样大小的页帧。再通过页表建立虚拟页和物理页帧的映射。

当CPU生成一个虚拟地址并且需要访问时，内存管理单元(MMU)将地址划分为页号(Page Number)(高位部分)和页内偏移(Offset)(地位部分)，再查询页表找到对应的**物理页帧号**，拼接物理地址(=物理页帧号*4096+页内偏移)，最后从物理地址中读取数据。

当我们想要查找最后一个元素的时候，可以使用`rbegin`函数，也可以使用`end`函数，但是要注意，end函数得到的指针不是最后一个数的指针，而是最后一个树后一位的指针，用于指示容器的结束。因此需要这样解引用：
...
这是STL中的一个关联式容器，用于储存键值对（key-value），其中每一个键都是唯一的，并且再map中会基于红黑树对键进行排列。

其中键Key的值是一个模板值，只要能实现`<`符号即可。可以使用数字、字符串、自定义类（需要进行`<`运算符重载）。

它的逻辑和set很想，把set节点中存储的内容想象为只有键的数据，那么map节点中就是添加了值的数据。

3. std::multimap/std::multiset

这两个是STL的关联容器中set和map的变体，他们之间本质区别只有，multi的关联容器可以存储同一个键的内容，仅此而已。如果需要只是排序但是不去重的场景可以考虑这两种关联容器。

#### 无序关联式容器（哈希）

这是C++11之后的新特性，基于引入的**哈希表**(`Hash Table`)实现。

##### 哈希函数和哈希表

什么是哈希表？我们从哈希函数说起。**哈希函数**是一种函数，可以将长度不同的数据映射为固定大小的值（通常是整数）。这个输出值就叫**哈希值**(hash value)或哈希码(hash code)。而**哈希表**是一种基于哈希函数实现的数据结构，通过哈希函数将键Key变为数组下标，从而实现O(1)时间复杂度的插入、删除和查找操作。

哈希表使用一个桶(Bucket)来存储数据，使用哈希函数`h(Key)`计算出键转化后的数组索引，然后将键和值放入桶中。如果多个Key映射到了同一个桶中，就叫做**哈希冲突**，需要通过某种策略解决（链地址法、开发寻址法）。在C++标准库中使用的是**链地址法**，将同一个桶中的数据使用链表的形式连接在一起。
...
Bucket N: → [key4, value4] → [key5, value5] → [key6, value6]
```

不过为什么它可以实现平均O(1)时间复杂度呢？

当我们需要程序寻找一个键对应的值（或者这个键本身），首先通过哈希函数得到桶序列O(1)，再从桶中找到键值对。假设哈希表中一共有n个键值对m个桶，那么一个桶中可能存放n/m个键值对。我们令这个比例为λ**负载因子**(Load Factor)。那么在查找元素时，最多查找λ个元素，而λ是一个常数，所以这个查找的部分时间复杂度也只有O(1)。
